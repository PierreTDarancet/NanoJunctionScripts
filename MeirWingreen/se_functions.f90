...
!***********************************************
   SUBROUTINE calcul_electron_photon_sigma(sigma_r, sigma_lesser, g_r_eneplusomega, glesser_eneplusomega, g_r_eneminusomega, glesser_eneminusomega, amplitude, omegaphoton, dipole_matrix, dimC, method)
   !***********************************************
   !   SUBROUTINE calcul_electron_photon_sigma(sigma_ep_r(ie,:,:), sigma_ep_lesser(ie,:,:), g_C(egridplusomega(ie),:,:), glesser(egridplusomega(ie),:,:), g_C(egridminusomega(ie),:,:), glesser(egridminusomega(ie),:,:), amplitude, photonfrequency, dipole_matrix(:,:),  dimC, ep_method  )
     ...
     COMPLEX(dbl)  ::  work4(dimC,dimC) 
         ...
         ENDIF 
          ...
          IF (TRIM(method)=="AM") THEN ! Aeberhard Morf PRB 77 (2008) - SCBA
              ...
              CALL mat_mul(work3, work4 , 'N', dipole_matrix, 'N', dimC, dimC, dimC)
              CALL mat_mul(work1, dipole_matrix, 'N', work3, 'N', dimC, dimC, dimC)
              sigma_lesser(:,:) = CI*work1(:,:)* (amplitude**(2))
          ELSE IF ((TRIM(method)=="GN").OR.(TRIM(method)=="G0") ) THEN ! Galperin Nitzan PRL 2005 with dipole transition/ Galperin Nitzan PRL 2005 without dipole transition -set to one
              work4(:,:) =  glesser_eneplusomega(:,:) +  glesser_eneminusomega(:,:)  
              CALL mat_mul(work3, work4 , 'N', dipole_matrix, 'N', dimC, dimC, dimC)
              CALL mat_mul(work1, dipole_matrix, 'N', work3, 'N', dimC, dimC, dimC)
              sigma_lesser(:,:) = CI*work1(:,:)* (amplitude**(2))
          ...
          IF (TRIM(method)=="AM") THEN ! Aeberhard Morf PRB 77 (2008) - SCBA
              ...
              work2(:,:) = work4(:,:) + work3(:,:)
              CALL mat_mul(work3, work2 , 'N', dipole_matrix, 'N', dimC, dimC, dimC)
              CALL mat_mul(work1, dipole_matrix, 'N', work3, 'N', dimC, dimC, dimC)
