       ...
       IF( .NOT. alloc ) THEN 
         ...
         ENDIF 
          ...
          DO ie = 1, ne_green
              ...
              IF ((green_egrid(ie) >= emin).AND.(green_egrid(ie) <= emax)) THEN
                     IF (( green_egrid(ie) >= egrid(ie_test-1) ) .AND. ( green_egrid(ie) < egrid(ie_test) ) ) THEN
                        grid_condtogreen(ie_test-1)=ie
                        ie_test=ie_test+1
                     ENDIF
              ENDIF
          ENDDO
   ...
   INTEGER      :: ie, ierr, ie_test
       ...
       IF( .NOT. alloc ) THEN 
         ...
         ENDIF 
          ...
          ie_test=1
          DO ie = 1, ne_green-1
              ene_min = green_egrid(ie_test)  -omega_photon
              ene_max = green_egrid(ie_test+1)  -omega_photon
              IF ((green_egrid(ie) >= ene_min ).AND. (green_egrid(ie) < ene_max)) THEN
                  egridminusomega(ie)=ie_test 
                  ie_test=ie_test+1
              ELSE
