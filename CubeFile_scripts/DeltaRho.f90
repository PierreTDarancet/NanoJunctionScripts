...
REAL, ALLOCATABLE :: chg(:,:,:,:), chgplane(:,:),chgave(:,:,:),chgsum(:),  chg2(:,:,:,:), chgplane2(:,:),chgave2(:,:,:),chgsum2(:),
	! chgave(iz,iave,is),posave(iz,iave,is)f
...
INTEGER, ALLOCATABLE :: lnn(:)
 CHARACTER(30) :: Fin,Fop, Fop2,Fout,A
 CHARACTER(60) :: atoms
...
READ(10,'(A)')Fop		! which Ref file to be read: CHG, pot 0.0
Fop=TRIM(Fop)
READ(10,'(A)')Fop2		! which Ref file to be read: CHG, pot finite bias
Fop=TRIM(Fop2)
...
OPEN(unit=12,FILE=Fop2,status='old',form='formatted')
PRINT*, "Opened"
READ(12,*) vec
PRINT*, "vec", vec 
READ(12,*) ngrid,ns
...
ALLOCATE(chg2(ngrid(1),ngrid(2),ngrid(3),ns))
ALLOCATE(chgplane2(ngrid(plane),ns))
ALLOCATE(chgsum2(ns))
