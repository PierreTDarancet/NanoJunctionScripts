...
CHARACTER(15) :: output_mode, output_type ! Disp or Amp --define output type
...
if (N_arg .ne. 8) then
   ...
   write(*,*) '7) Output mode normed Disp/Amp/ModeDisp'
   write(*,*) '8) Output Type: nmd/xyz/fdf' 
   write(*,*) '9) Output Scaling Factor (Ang/sqrt(aum)Ang)'
   write(*,*) '------------------------------------'
   write(*,*) 'Input'
...
CALL GETARG(7, STR_arg) ! 7) Output mode: 
 read(STR_arg,*) output_mode
 IF ( TRIM(output_mode) == 'Disp' ) THEN
    IF ( debug ) THEN
        WRITE(*,*) "Reading arguments: Output mode Disp"
        WRITE(iout,*)  "Reading arguments: Output mode Disp"
    END IF
 ELSE IF ( TRIM(output_mode) == 'Amp' ) THEN
    IF ( debug ) THEN
        WRITE(*,*) "Reading arguments: Output mode Amp"
        WRITE(iout,*)  "Reading arguments: Output mode Amp"
    END IF
 ELSE IF ( TRIM(output_mode) == 'ModeDisp' ) THEN 
    IF ( debug ) THEN
        WRITE(*,*) "Reading arguments: Output mode ModeDisp"
        WRITE(iout,*) "Reading arguments: Output mode ModeDisp"
    END IF
 ELSE
     WRITE(*,*) 'Error: output_mode:', TRIM(output_mode)
     WRITE(*,*) 'Not understood'
     WRITE(iout,*) 'Error: output_mode:', TRIM(output_mode)
     WRITE(iout,*) 'Not understood'
     STOP
 END IF

CALL GETARG(8, STR_arg) ! 8) Output Type: nmd/xyz/fdf' 
read(STR_arg,*) output_type
IF ( TRIM(output_type) == 'xyz' ) THEN
   IF ( debug ) THEN
       WRITE(*,*) "Reading arguments: Output type xyz"
       WRITE(iout,*)  "Reading arguments: Output type xyz"
   END IF
ELSE IF ( TRIM(output_type) == 'fdf' ) THEN
   IF ( debug ) THEN
       WRITE(*,*) "Reading arguments: Output type fdf"
       WRITE(iout,*)  "Reading arguments: Output type fdf"
   END IF
ELSE IF ( TRIM(output_type) == 'nmd' ) THEN 
   IF ( debug ) THEN
       WRITE(*,*) "Reading arguments: Output type nmd"
       WRITE(iout,*) "Reading arguments: Output type nmd"
   END IF
ELSE
    WRITE(*,*) 'Error: OutputType:', TRIM(output_type)
    WRITE(*,*) 'Not understood'
    WRITE(iout,*) 'Error: OutputType:', TRIM(output_type)
    WRITE(iout,*) 'Not understood'
    STOP
END IF

CALL GETARG(9, STR_arg) !9) Output Scaling Factor (Ang/sqrt(aum)Ang)
...
IF ( TRIM(output_type) == 'nmd' ) THEN
    ...
    ENDIF
    filename="mod.nmd" ! to be generalized
...
ELSE 
    ...
    do Mode=1, N, 1 ! Modeloop
        ...
        IF ( TRIM(output_mode) == 'ModeDisp' ) THEN
            IF ( debug ) THEN
                WRITE(*,*) "Writing Mode", Mode,'with norm Mode to file', TRIM(filename), 'type',  TRIM(output_mode)
                WRITE(iout,*) "Writing Mode", Mode,'with norm Mode to file', TRIM(filename), 'type',  TRIM(output_mode)
            END IF
            CALL writeoutmode(imodem,filename,TRIM(output_type),Atoms(1:Nat),A(:,Mode),ModeNorm(Mode),Nat,StaticAtN,ndim)
            IF ( debug ) THEN
                WRITE(*,*) "Writing Mode.... Done"
                WRITE(iout,*) "Writing Mode.... Done"
            END IF
        ELSE IF  ( TRIM(output_mode) == 'Disp' ) THEN  
            IF ( debug ) THEN
                WRITE(*,*) "Writing Mode", Mode,'with norm Disp to file', TRIM(filename), 'type',  TRIM(output_mode)
                WRITE(iout,*) "Writing Mode", Mode,'with norm Disp to file', TRIM(filename), 'type',  TRIM(output_mode)
            END IF
            CALL writeoutmode(imodem,filename,TRIM(output_type),Atoms(1:Nat),A(:,Mode),(ScaleDisp/displacement(Mode)),Nat,StaticAtN,ndim)
            IF ( debug ) THEN
                WRITE(*,*) "Writing Mode.... Done"
                WRITE(iout,*) "Writing Mode.... Done"
            END IF
        ELSE IF  ( TRIM(output_mode) == 'Amp' ) THEN  
            IF ( debug ) THEN
                WRITE(*,*) "Writing Mode", Mode,'with norm Amp to file', TRIM(filename), 'type',  TRIM(output_mode)
                WRITE(iout,*) "Writing Mode", Mode,'with norm Amp to file', TRIM(filename), 'type',  TRIM(output_mode)
            END IF
            CALL writeoutmode(imodem,filename,TRIM(output_type),Atoms(1:Nat),A(:,Mode),ScaleDisp,Nat,StaticAtN,ndim)
            IF ( debug ) THEN
                WRITE(*,*) "Writing Mode.... Done"
                WRITE(iout,*) "Writing Mode.... Done"
            END IF
...
SUBROUTINE writeoutmode(ifile,filename,otype,Atoms,Mode,ScaleFactor,Nat,StaticAtN,dimn)
use constants
use type_mod, ONLY  atom  
IMPLICIT NONE
...
TYPE(atom), INTENT(in) :: Atoms(Nat+StaticAtN)
REAL(dp), INTENT(in) :: Mode(dimn*Nat)
REAL(dp), INTENT(in) :: ScaleFactor
CHARACTER(30), INTENT(in) :: filename
CHARACTER(30), INTENT(in) :: otype
...
open(UNIT=ifile, file=TRIM(filename),form='formatted',status='unknown')

IF ( TRIM(otype) == 'xyz' ) THEN
    IF ( debug ) THEN
        WRITE(*,*) "Reading arguments: Output type xyz"
    END IF
    write(ifile,*) (Nat+StaticAtN)
    write(ifile,*) 'Scaling Factor:', ScaleFactor
    do i=1, Nat, 1
        write(ifile,'(a2,6f14.8)') Atoms(i)%name, &
           Atoms(i)%r(:)*BOHR_ANG,&
           Mode((1+ndim*(i-1)):(ndim*i))*BOHR_ANG*ScaleFactor
    end do
    do i=Nat+1, Nat+StaticAtN,1
        write(ifile,'(a2,6f14.8)') Atoms(i)%name, &
           Atoms(i)%r(:)*BOHR_ANG,&
           ZERO, ZERO, ZERO 
    end do
ELSE IF ( TRIM(otype) == 'fdf' ) THEN
    IF ( debug ) THEN
        WRITE(*,*) "Reading arguments: Output type fdf"
    END IF
    write(ifile,*)'#frequency cm-1 ', eigv(Mode)*THz_CM/TWO/PI
    write(ifile,*) '# Scaling factor:', ScaleFactor 
    write(ifile,*) 'AtomicCoordinatesFormat NotScaledCartesianAng'
    write(ifile,*) '%block AtomicCoordinatesAndAtomicSpecies'
    do i=1, Nat, 1 
        write(ifile,'(3f14.8, i4,"  ",a2,"  ",i4)')  &
            (Atoms(i)%r(1) - (ScaleFactor*Mode(1+ndim*(i-1)))*BOHR_ANG, &
            (Atoms(i)%r(3) - (ScaleFactor*Mode(2+ndim*(i-1)))*BOHR_ANG, &
            (Atoms(i)%r(3) - (ScaleFactor*Mode(3+ndim*(i-1)))*BOHR_ANG, &
            Atoms(i)%type, &
            Atoms(i)%name, &
            i
    end do
    do i=Nat+1, Nat+StaticAtN,1
        write(ifile,'(3f14.8, i4,"  ",a2,"  ",i4)')  &
            (Atoms(i)%r(1))*BOHR_ANG, &
            (Atoms(i)%r(3))*BOHR_ANG, &
            (Atoms(i)%r(3))*BOHR_ANG, &
            Atoms(i)%type, &
            Atoms(i)%name, &
            i
    end do
    write(ifile,*) '%endblock AtomicCoordinatesAndAtomicSpecies'
ELSE
    WRITE(*,*) 'Error: OutputType:', TRIM(otype)
    WRITE(*,*) 'Not valid'
    STOP
END IF
