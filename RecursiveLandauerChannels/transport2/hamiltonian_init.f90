...
!*******************************************************************
   ...
   COMPLEX(dbl), ALLOCATABLE :: aux3(:,:,:)
   COMPLEX(dbl), ALLOCATABLE :: aux4(:,:,:)
...
!
   ...
   ALLOCATE( aux(dimC,dimC,nrtot_par), STAT=ierr)
      IF ( ierr/=0 ) CALL errore(subname,'allocating aux',ABS(ierr))
   ALLOCATE( aux2(dimC,dimR,nrtot_par), STAT=ierr)
      IF ( ierr/=0 ) CALL errore(subname,'allocating aux2',ABS(ierr))
   ALLOCATE( aux3(dimL,dimC,nrtot_par), STAT=ierr)
      IF ( ierr/=0 ) CALL errore(subname,'allocating aux3',ABS(ierr))
   ALLOCATE( aux4(dimL,dimL,nrtot_par), STAT=ierr)
      IF ( ierr/=0 ) CALL errore(subname,'allocating aux4',ABS(ierr))
   ...
   !
   CALL read_matrix( datafile_C, 'H00_C', dimC, dimC, aux, dimC, dimC)
   CALL fourier_par(h00_C, dimC, dimC, aux, dimC, dimC)   
   !
   CALL read_matrix( datafile_C, 'H_CR', dimC, dimR, aux2, dimC, dimR)
   CALL fourier_par(h_CR, dimC, dimR, aux2, dimC, dimR)   
   ...
   CASE ( "conductor" )
       ...
       !
       CALL read_matrix( datafile_C, 'H_LC', dimL, dimC, aux3, dimL, dimC)
       CALL fourier_par (h_LC, dimL, dimC, aux3, dimL, dimC)   
       !
       CALL read_matrix( datafile_L, 'H00_L', dimL, dimL, aux4, dimL, dimL)
       CALL fourier_par (h00_L, dimL, dimL, aux4, dimL, dimL)   
       !
       !CALL read_matrix( datafile_L, 'H01_L', dimL, dimL, aux4, dimL, dimL)
       !CALL fourier_par (h01_L, dimL, dimL, aux4, dimL, dimL)   
       !
       CALL read_matrix( datafile_R, 'H00_R', dimR, dimR, aux4, dimR, dimR)
       CALL fourier_par (h00_R, dimR, dimR, aux4, dimR, dimR)   
       !
       !CALL read_matrix( datafile_R, 'H01_R', dimR, dimR, aux4, dimR, dimR)
       !CALL fourier_par (h01_R, dimR, dimR, aux4, dimR, dimR)   
       !
   ...
   DEALLOCATE( aux3, STAT=ierr)
      IF ( ierr/=0 ) CALL errore(subname,'deallocating aux3',ABS(ierr))
   DEALLOCATE( aux4, STAT=ierr)
      IF ( ierr/=0 ) CALL errore(subname,'deallocating aux4',ABS(ierr))
