...
! SUBROUTINE   mat_mul( c, a, opa, b, opb, m, n, k)
   ...
   !
   COMPLEX(dbl)     :: ene
   INTEGER         :: i,ie,ibias, ivoc,ierr,ncount

!   REAL(dbl),    ALLOCATABLE :: dos(:,:,:,:), conduct(:,:,:,:)
!   REAL(dbl), ALLOCATABLE :: gamma_R_safe(:,:,:,:,:), gamma_L_safe(:,:,:,:,:)
   REAL(dbl),    ALLOCATABLE :: cond_aux(:), total_conduct(:)
   COMPLEX(dbl), ALLOCATABLE :: work(:,:), work2(:,:)

! Contains Model Hamiltonian data
! 
   REAL(dbl)       ::   HOMO_left, HOMO_right
   REAL(dbl)       ::   LUMO_left, LUMO_right
   COMPLEX(dbl)    ::   H_lL
   COMPLEX(dbl)    ::     H_lR
   COMPLEX(dbl)    ::    H_rL
   COMPLEX(dbl)    ::    H_rR
...
! Contains transport bias-grid data
! 
   ...
   !
  ! LOGICAL :: biasgridalloc = .FALSE.

! Contains transport energy-grid data
! 

   INTEGER                :: ne        ! max dimension of the energy grid
   INTEGER                :: ne_bias        ! dimension of the energy grid for each bias
...
! Contains transport HOMO-D-LUMO-A-grid data
! 
   
   INTEGER                :: nvoc        ! dimension of the energy grid
   REAL(dbl)              :: vocmin      !
   REAL(dbl)              :: vocmax      ! Voc grid extrema 
   !
   REAL(dbl):: dvoc
   REAL(dbl), ALLOCATABLE :: vocgrid(:)  ! grid values
   !
 !  LOGICAL :: vocgridalloc = .FALSE.
...
!
   ...
   ! Entry variables
   !
   PRINT*, 'Beginning of Hamiltonian initialization'

     HOMO_left=-4.00             ! indicated wrt to \mu_L
     LUMO_left=2.00              ! Let's take Left as the Acceptor
   ...
   ! READING OR NOT THE INIT VALUE.......
   PRINT*, 'Beginning grid parameter initialization'
...
!  DEALLOCATE ( conduct , STAT=ierr )
 ...
 ! DEALLOCATE ( gamma_R_safe , STAT=ierr )
  ...
  PRINT*, "Deallocate transport results"
