   ...
   IMPLICIT NONE
     ...
     !      CALL errore(subname,'datafile_C unspecified',1)
      ...
      IF ( (dimC <= 0)  .AND. (TRIM(datafile_C_form)=='hamiltonian') ) CALL errore(subname,'Invalid dimC',10)
           !
      IF ( LEN_TRIM(in_datafile_gamma_R) == 0 ) &
            CALL errore(subname,'datafile_gamma_R unspecified',12)
      IF ( LEN_TRIM(in_datafile_gamma_L) == 0 ) &
            CALL errore(subname,'datafile_gamma_L unspecified',12)
      IF ( LEN_TRIM(in_datafile_sigma_R) == 0 ) &
            CALL errore(subname,'datafile_sigma_R unspecified',12)
      IF ( LEN_TRIM(in_datafile_sigma_L) == 0 ) &
            CALL errore(subname,'datafile_sigma_L unspecified',12)
