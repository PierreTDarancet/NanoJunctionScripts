...
!*********************************************
   SUBROUTINE  green_CC_tridiag( g_CC_1N, g_CC_N1, dos,  ene, A_matrix_in, B_matrix, sgm_L, sgm_R, max_iter, dim_sub) 
!*********************************************
...
! d'une restriction du Hamiltonien
   ...
   REAL(dbl),    INTENT(out)   :: dos(dim_sub)
   !
   ! local variables
   !
...
!
   ...
   g_CC_N1(:,:) = g_offdiag(1,:,:)
   !!!!!!!!!!!!!! ATTTTTTTTTTTEEEEEEEEEENNNNTTION
   ! CECI N'est pas UNE DOS, MAIS SIMPLEMENT UNE PROJECTION SUR SOUS ESPACE
   DO i_sub = 1, dim_sub
      dos(i_sub) = - AIMAG(g_diag(1,i_sub, i_sub)) /PI
   ENDDO
   !!!!!!!!!
