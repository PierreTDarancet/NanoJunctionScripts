...
!***********************************************
   SUBROUTINE Molecular_hamiltonian_init(h00_C, h_CR, h_LC, h_RC, h_CL,  h00_L, h01_L, h00_R, h01_R, omega_max, ihomo, ilumo, ehomo, elumo, gamma_ratio, dimCaux, dimLaux, dimRaux,   debug_level)  
   !***********************************************
...
!      REAL(dbl), INTENT(out)  :: elumo(INT(dimCaux/2))
      ...
      IF ( dimCaux == 2) THEN
	      ...
	      h_CR(1,1)  = sqrt( realaux / (ONE +  (ONE/gamma_ratio) ) )
	      h_LC(1,1)  =  sqrt( realaux / (ONE +   gamma_ratio ) )

	      realaux = h_CR_ref(2,1)**2 +  h_LC_ref(1,2)**2

	      h_CR(2,1)  = sqrt( realaux / (ONE +   gamma_ratio     ) )
	      h_LC(1,2)  =  sqrt( realaux / (ONE + (ONE/gamma_ratio)) )
