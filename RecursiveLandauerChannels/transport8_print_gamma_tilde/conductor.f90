...
!
!***********************************************
   PROGRAM conductor8_print_gamma_tilde
   !***********************************************
   ...
   USE T_gamma_module,       ONLY : gamma_allocate, gamma_L_print, gamma_R_print, &
   ...
   !
   REAL(dbl),    ALLOCATABLE :: sigma_test_R(:,:), sigma_test_L(:,:)
...
!          IF( ierr /=0 ) CALL errore('conductor','allocating dos_CC', ABS(ierr) )
   ...
   ALLOCATE ( dos_L(dim_subspace,ne), STAT=ierr )
      ...
      !
   ALLOCATE ( sigma_test_R(dim_subspace,ne), STAT=ierr )
      IF( ierr /=0 ) CALL errore('conductor','allocating sigma_R', ABS(ierr) )
   ALLOCATE ( sigma_test_L(dim_subspace,ne), STAT=ierr )
...
!                WRITE( stdout,"(2x,'Energy step = ',i5) ") ncount
      ...
      IF ( print_gamma ) THEN 
            ...
            sigma_R(ie,:,:) =  sgm_CR(:,:)
            !
            gamma_R_s(ie,:,:) = gamma_tilde_R(:,:)
            !
            gamma_L_s(ie,:,:) = gamma_tilde_L(:,:)
            !
      ENDIF
      ...
      !
      sigma_test_L(:,ie) = ZERO
      DO i = 1, dim_subspace
         sigma_test_L(i,ie) =  -  REAL(SUM( sgm_L(:,i) ))
      ENDDO
      !
      sigma_test_R(:,ie) = ZERO
      DO i = 1, dim_subspace
         sigma_test_R(i,ie) =  - REAL(SUM( sgm_R(:,i) ))
      ENDDO
      !
      sigma_CL(:,ie) = ZERO
      DO i = 1, dim_subspace
   ...
   IF ( print_gamma ) THEN 
         ...
         OPEN ( cond_unit, FILE=TRIM(filename), FORM='formatted' )
         CALL sigma_L_print(cond_unit,TRIM(filename))
         CLOSE( cond_unit )
         !
         filename = 'sigma_R.dat'
         OPEN ( cond_unit, FILE=TRIM(filename), FORM='formatted' )
         CALL sigma_R_print(cond_unit,TRIM(filename))
         CLOSE( cond_unit )
         !
   ENDIF   
...
!
   ...
   ENDDO
   CLOSE( cond_unit )
   filename = 'sigma_test_L.dat'
   OPEN ( dos_unit, FILE=TRIM(filename), FORM='formatted' )
   DO ie = 1, ne
       WRITE ( dos_unit, '(10(f15.9))' ) egrid(ie), SUM( sigma_test_L(:,ie) ), (sigma_test_L(i_sub,ie), i_sub=1,dim_subspace )
   ENDDO
   CLOSE( dos_unit )
   filename = 'sigma_test_R.dat'
   OPEN ( dos_unit, FILE=TRIM(filename), FORM='formatted' )
   DO ie = 1, ne
       WRITE ( dos_unit, '(10(f15.9))' ) egrid(ie), SUM( sigma_test_R(:,ie) ), (sigma_test_R(i_sub,ie), i_sub=1,dim_subspace )
   ENDDO
   CLOSE( dos_unit )
...
!
   ...
   DEALLOCATE ( sigma_test_L, STAT=ierr )
        IF( ierr /=0 ) CALL errore('conductor','deallocating sigma_L', ABS(ierr) )
   DEALLOCATE ( sigma_test_R, STAT=ierr )
...
END PROGRAM conductor8_print_gamma_tilde
