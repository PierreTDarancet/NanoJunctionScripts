...
!*******************************************************************
   ...
   ALLOCATE ( final_hamiltonian_LC(dim_subspace,dimC), STAT=ierr )
        IF( ierr /=0 ) CALL errore(subname, 'allocating final_hamiltonian_LC', ABS(ierr) )
   ALLOCATE ( final_hamiltonian_CR(dimC,dim_subspace), STAT=ierr )
...
!       ENDDO
     ...
     DO icols=1, dim_subspace
        DO irows=1, dim_subspace
           final_hamiltonian_L(2,irows,icols)=B_matrix(1,irows,icols)
        ENDDO
     ENDDO
