...
!*********************************************
   ...
   !USE parser_module
   USE iotk_module
   USE convert4_control_variable_module,      ONLY : out_datafile_C, &
                                            out_datafile_L, &
                                            out_datafile_R, &
                                            in_datafile_L, &
                                            in_datafile_R, &
   ...
   ! general functions
   PUBLIC :: matrix4_allocate
   PUBLIC :: matrix4_deallocate
   PUBLIC :: print4_want
   PUBLIC :: read4_matrix
   PUBLIC :: build4_hamiltonian
   PUBLIC :: build4_matrix
   PUBLIC :: convert4_summary_input
...
!*******************************************************************
   SUBROUTINE matrix4_allocate
   !*******************************************************************
      CHARACTER(16)      :: subname="matrix4_allocate"
      INTEGER :: ierr
   ...
   ALLOCATE ( final_hamiltonian_R(dimR,dimR), STAT=ierr )
        IF( ierr /=0 ) CALL errore(subname, 'allocating final_hamiltonian_R', ABS(ierr) )
   ALLOCATE ( final_hamiltonian_L(dimL,dimL), STAT=ierr )
...
END SUBROUTINE matrix4_allocate

!**********************************************************
   SUBROUTINE matrix4_deallocate()
   !**********************************************************
   IMPLICIT NONE
      CHARACTER(18)      :: subname="matrix4_deallocate"
      INTEGER :: ierr
   ...
   END SUBROUTINE matrix4_deallocate


!*******************************************************************
   SUBROUTINE build4_hamiltonian
   !*******************************************************************
       CHARACTER(18)      :: subname="build4_hamiltonian"
       INTEGER :: icols, irows 
       INTEGER :: iter
       INTEGER :: icols_shift, irows_shift
       INTEGER :: range_iter, shift_iter
     ! Beginning
     ...
     DO iter=2,range_iter
         ...
         DO icols=1, dim_subspace
             ...
             DO irows=1, dim_subspace
                ...
                !
                final_hamiltonian_L(icols_shift,irows_shift)=CONJG(B_matrix(shift_iter,irows,icols))
                !
                final_hamiltonian_L(irows_shift,icols_shift)=B_matrix(shift_iter,irows,icols)
                !
             ENDDO
             !
         ENDDO
     ...
     DO iter=1,range_iter
         ...
         DO icols=1, dim_subspace
            ...
            DO irows=1, dim_subspace
               ...
               !
               final_hamiltonian_L(irows_shift,icols_shift)=A_matrix(shift_iter,irows,icols)
               !
            ENDDO
            !
         ENDDO
     ...
     DO iter=(out_iter_L + out_iter_C + 2) ,(out_iter_L + out_iter_C + range_iter)
      DO icols=1, dim_subspace
         ...
         DO irows=1, dim_subspace
            ...
            final_hamiltonian_R(irows_shift,icols_shift)=B_matrix(iter,irows,icols)
            final_hamiltonian_R(icols_shift,irows_shift)=CONJG(B_matrix(iter,irows,icols))
            !
         ENDDO
      ENDDO
     ENDDO
     ...
     DO iter=(out_iter_L+out_iter_C+1) ,(out_iter_L+out_iter_C+range_iter)
      DO icols=1, dim_subspace
         ...
         DO irows=1, dim_subspace
            ...
            !
            final_hamiltonian_R(irows_shift,icols_shift)=A_matrix(iter,irows,icols)
            !
         ENDDO
      ENDDO
     ENDDO
  ...
  END SUBROUTINE build4_hamiltonian


!*******************************************************************
   SUBROUTINE build4_matrix
   !*******************************************************************
     ...
     ! starting from the knowledge of the A/B_matrix_L/R read from recursion's files
       CHARACTER(13)      :: subname="build4_matrix"
...
!             ENDDO
   ...
   END SUBROUTINE build4_matrix



!**********************************************************
   SUBROUTINE print4_want
   !**********************************************************
   IMPLICIT NONE
     ...
     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! initialization PART
      ...
      CALL file_open(ham_unit,TRIM(out_datafile_R),PATH="/",ACTION="write", FORM='formatted')
       ...
       !
       CALL iotk_write_begin(ham_unit,"RHAM")
       CALL iotk_write_dat(ham_unit,"VR"//TRIM(iotk_index(ir)), final_hamiltonian_R(:,:))
       CALL iotk_write_end(ham_unit,"RHAM")
       CALL iotk_write_end(ham_unit,TRIM(name))
       !
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      CALL file_open(ham_unit,TRIM(out_datafile_L),PATH="/",ACTION="write", FORM='formatted')
       ...
       !
       CALL iotk_write_begin(ham_unit,"RHAM")
        CALL iotk_write_dat(ham_unit,"VR"//TRIM(iotk_index(ir)), final_hamiltonian_L(:,:))
       CALL iotk_write_end(ham_unit,"RHAM")
       CALL iotk_write_end(ham_unit,TRIM(name))
       !
       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   END SUBROUTINE print4_want


!*******************************************************************
   SUBROUTINE read4_matrix
   !*******************************************************************
       CHARACTER(12)      :: subname="read4_matrix"
  ...
  END SUBROUTINE read4_matrix

!**********************************************************
   SUBROUTINE convert4_summary_input
   !**********************************************************
   ! 
...
!          ENDIF
   ...
   END SUBROUTINE convert4_summary_input





 END MODULE convert4_module
