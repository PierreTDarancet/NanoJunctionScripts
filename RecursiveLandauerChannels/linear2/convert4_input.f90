...
!**********************************************************
   SUBROUTINE convert4_input_manager()
   !**********************************************************
      USE convert4_input_parameters_module,  ONLY :convert4_read_namelist_control

     IMPLICIT NONE
      INTEGER :: ierr
      CHARACTER(22) :: subname='convert4_input_manager'
      ...
      CALL convert4_read_namelist_control(stdin)
      ...
      !
      CALL convert4_setup_control()
      CALL convert4_setup_io()
      !CALL convert_setup_dim()
      !CALL convert_setup_subspace()


   END SUBROUTINE convert4_input_manager

!**********************************************************
   SUBROUTINE convert4_setup_control()
   !**********************************************************
      USE convert4_control_variable_module,     ONLY :    out_datafile_C, out_datafile_L, out_datafile_R, &
                                                         in_datafile_L, in_datafile_R, &
                                                         dim_subspace, &
                                                         n_iter_R, n_iter_L, &
                                                         out_iter_R, out_iter_L, &
                                                         out_iter_C
      USE convert4_input_parameters_module, ONLY :   out_datafile_C_ => out_datafile_C, &
   ...
   END SUBROUTINE convert4_setup_control
! 
! !**********************************************************
...
!**********************************************************
   SUBROUTINE convert4_setup_io()
   !**********************************************************
      USE io_module,                ONLY : prefix, postfix, work_dir, title
      USE convert4_input_parameters_module,  ONLY : prefix_    => prefix,       &
                                           postfix_   => postfix,      &
                                           work_dir_  => work_dir,     &
                                           title_     => title
      IMPLICIT NONE
      ...
      work_dir = work_dir_
      title    = title_
   END SUBROUTINE convert4_setup_io


END MODULE convert4_input_module
