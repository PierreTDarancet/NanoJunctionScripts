...
!      IF( ierr /=0 ) CALL errore('conductor','allocating PN_g_CC_P1', ABS(ierr) )
   ALLOCATE ( gC((dim_subspace*max_iter_CC),(dim_subspace*max_iter_CC)), STAT=ierr )
      IF( ierr /=0 ) CALL errore('conductor','allocating gC', ABS(ierr) )
...
!                WRITE( stdout,"(2x,'Energy step = ',i5) ") ncount
      ...
      ! Calcul de G_CC
      !
      CALL green_CC_tridiag( gC, ene, A_matrix_C(:,:,:), B_matrix_C(:,:,:), sgm_CL(:,:), sgm_CR(:,:), max_iter_CC, dim_subspace) 


      !
      CALL transmittance_min(dim_subspace, max_iter_CC, gamma_tilde_L, gamma_tilde_R, gC(:,:), conduct(:,ie) )
      !
...
!
   ...
   DO ie = 1, ne
       WRITE ( cond_unit, '(7(f15.9))' ) egrid(ie), SUM( conduct(:,ie) ), (conduct(i_sub,ie), i_sub=1,dim_subspace)
   ENDDO
   CLOSE( cond_unit )
   filename = 'sigma_L.dat'
   OPEN ( dos_unit, FILE=TRIM(filename), FORM='formatted' )
   DO ie = 1, ne
       WRITE ( dos_unit, '(7(f15.9))' ) egrid(ie), SUM( sigma_L(:,ie) ), (sigma_L(i_sub,ie), i_sub=1,dim_subspace )
   ENDDO
   CLOSE( dos_unit )
   filename = 'sigma_R.dat'
   OPEN ( dos_unit, FILE=TRIM(filename), FORM='formatted' )
   DO ie = 1, ne
       WRITE ( dos_unit, '(7(f15.9))' ) egrid(ie), SUM( sigma_R(:,ie) ), (sigma_R(i_sub,ie), i_sub=1,dim_subspace )
   ENDDO
   CLOSE( dos_unit )
   filename = 'sigma_CL.dat'
   OPEN ( dos_unit, FILE=TRIM(filename), FORM='formatted' )
   DO ie = 1, ne
       WRITE ( dos_unit, '(7(f15.9))' ) egrid(ie), SUM( sigma_CL(:,ie) ), (sigma_CL(i_sub,ie), i_sub=1,dim_subspace )
   ENDDO
   CLOSE( dos_unit )
   filename = 'sigma_CR.dat'
   OPEN ( dos_unit, FILE=TRIM(filename), FORM='formatted' )
   DO ie = 1, ne
       WRITE ( dos_unit, '(7(f15.9))' ) egrid(ie), SUM( sigma_CR(:,ie) ), (sigma_CR(i_sub,ie), i_sub=1,dim_subspace )
   ENDDO
   CLOSE( dos_unit )
   filename = 'gamma_R.dat'
   OPEN ( dos_unit, FILE=TRIM(filename), FORM='formatted' )
   DO ie = 1, ne
       WRITE ( dos_unit, '(7(f15.9))' ) egrid(ie), SUM( gamma_R_sum(:,ie) ), (gamma_R_sum(i_sub,ie), i_sub=1,dim_subspace )
   ENDDO
   CLOSE( dos_unit )
   filename = 'gamma_L.dat'
   OPEN ( dos_unit, FILE=TRIM(filename), FORM='formatted' )
   DO ie = 1, ne
       WRITE ( dos_unit, '(7(f15.9))' ) egrid(ie), SUM( gamma_L_sum(:,ie) ), (gamma_L_sum(i_sub,ie), i_sub=1,dim_subspace )
   ENDDO
   CLOSE( dos_unit )
   filename = 'gamma_tilde_R.dat'
   OPEN ( dos_unit, FILE=TRIM(filename), FORM='formatted' )
   DO ie = 1, ne
       WRITE ( dos_unit, '(7(f15.9))' ) egrid(ie), SUM( gamma_tilde_R_sum(:,ie) ), (gamma_tilde_R_sum(i_sub,ie), i_sub=1,dim_subspace )
   ENDDO
   CLOSE( dos_unit )
   filename = 'gamma_tilde_L.dat'
   OPEN ( dos_unit, FILE=TRIM(filename), FORM='formatted' )
   DO ie = 1, ne
       WRITE ( dos_unit, '(7(f15.9))' ) egrid(ie), SUM( gamma_tilde_L_sum(:,ie) ), (gamma_tilde_L_sum(i_sub,ie), i_sub=1,dim_subspace )
   ENDDO
   CLOSE( dos_unit )
   filename = 'dos_L.dat'
   OPEN ( dos_unit, FILE=TRIM(filename), FORM='formatted' )
   DO ie = 1, ne
       WRITE ( dos_unit, '(7(f15.9))' ) egrid(ie), SUM( dos_L(:,ie) ), ( dos_L(i_L,ie), i_L=1,dim_subspace )
   ENDDO
   CLOSE( dos_unit )
   filename = 'dos_R.dat'
   OPEN ( dos_unit, FILE=TRIM(filename), FORM='formatted' )
   DO ie = 1, ne
       WRITE ( dos_unit, '(7(f15.9))' ) egrid(ie), SUM( dos_R(:,ie) ), ( dos_R(i_R,ie), i_R=1,dim_subspace )
   ENDDO
   CLOSE( dos_unit )
   filename = 'dos_CL.dat'
   OPEN ( dos_unit, FILE=TRIM(filename), FORM='formatted' )
...
!
 ...
 !       IF( ierr /=0 ) CALL errore('conductor','deallocating PN_g_CC_P1', ABS(ierr) )
   DEALLOCATE ( gC, STAT=ierr )
        IF( ierr /=0 ) CALL errore('conductor','deallocating gC', ABS(ierr) )

   CALL cleanup()

END PROGRAM conductor5_diag
