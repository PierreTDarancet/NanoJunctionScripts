...
!=====================================================
   ...
   USE summary2_module,     ONLY : summary2
   USE parser_module
   !
   ...
   INTEGER :: iter, i_sub, i_recur
   INTEGER, ALLOCATABLE :: rec2tot(:)
...
!
      ...
      CALL ioname('wannier_center',filename)
            ...
            CALL iotk_scan_attr(attr,"dimwann",dimwan, IERR=ierr)
            ...
            ALLOCATE (  rec2tot(dim_recursion), STAT=ierr ) 
               IF( ierr /=0 ) CALL errore(subname, 'allocating rec2tot ', ABS(ierr) )


            CALL iotk_scan_dat(aux_unit,"REC2TOT", rec2tot(:))
            !
            !
       CALL file_close(aux_unit,PATH="/CENTERS/",ACTION="read")


       ALLOCATE (  coeff(n_iter, dim_recursion), STAT=ierr ) 
               IF( ierr /=0 ) CALL errore(subname, 'allocating coeff ', ABS(ierr) )

       ALLOCATE (  state(n_iter,dim_subspace, dim_recursion), STAT=ierr ) 
               IF( ierr /=0 ) CALL errore(subname, 'allocating state ', ABS(ierr) )



       CALL ioname('wannier_center',filename,LPATH=.FALSE.)
       WRITE( stdout,"(/,' Recursion data read from file : ',a)") TRIM(filename)
           ...
           CALL file_open(aux2_unit,TRIM(filename),PATH="/STATES/",ACTION="read", &
            ...
            !
               DO i_sub=1,dim_subspace
                  CALL iotk_scan_dat(aux2_unit,"ITER"//TRIM(iotk_index(iter))//TRIM(iotk_index(i_sub)), state(iter,i_sub,:))
               ENDDO
            !
            ENDDO
            !
      ...
      !
      CALL summary2( stdout, LINPUT=.FALSE., LATOMS=.FALSE., LEIG=.FALSE. )
...
!
      ...
      WRITE( stdout,"(/,2x,'Calculate coeff  :')") TRIM(datatype)
      coeff(:,:) = CZERO
      ...
      !
         DO i_recur=1, dim_recursion
            ...
            DO i_sub=1, dim_subspace
               !
               coeff(iter,i_recur) = coeff(iter,i_recur) + (state(iter,i_sub,i_recur)*state(iter,i_sub,i_recur))
...
!
   ...
   DO iter=1, n_iter
      ...
      CASE( "modulus" )    
            ...
            DO i_recur = 1, dim_recursion
               !
               rwann_out(:,:,:) = rwann_out(:,:,:) + coeff(iter, i_recur) * REAL( cwann(:,:,:,rec2tot(i_recur)) * CONJG( cwann(:,:,:,rec2tot(i_recur)) ) )
            ENDDO 
      CASE( "real" )    
         str = "_WFR"
         DO i_recur = 1, dim_recursion
               !
               rwann_out(:,:,:) = rwann_out(:,:,:) + REAL (coeff(iter, i_recur) *  cwann(:,:,:,rec2tot(i_recur)))
         ENDDO 
      CASE( "imaginary" )    
         str = "_WFI"
         rwann_out(:,:,:) = AIMAG( cwann(:,:,:,i_recur) )
         DO i_recur = 1, dim_recursion
               !
               rwann_out(:,:,:) = rwann_out(:,:,:) + AIMAG (coeff(iter, i_recur) *  cwann(:,:,:,rec2tot(i_recur)))
         ENDDO 
      CASE( "modulnocoeff" )    
            str = "_WFM"
            DO i_recur = 1, dim_recursion
               ...
               IF (ABS(coeff(iter, i_recur)) >= EPS_m6) THEN
                   rwann_out(:,:,:) = rwann_out(:,:,:) + ONE * REAL( cwann(:,:,:,rec2tot(i_recur)) * CONJG( cwann(:,:,:,rec2tot(i_recur)) ) )
               ENDIF
            ENDDO 
      CASE DEFAULT
