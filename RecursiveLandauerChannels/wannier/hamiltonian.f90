...
!*********************************************
   ...
   USE localization_module, ONLY : rave
   IMPLICIT NONE
   PRIVATE
   SAVE
...
! SUBROUTINE hamiltonian_allocate2()
! SUBROUTINE hamiltonian_deallocate2()
! SUBROUTINE hamiltonian_write2(unit,name)
! SUBROUTINE hamiltonian_read2(unit,name,found)
!
...
!**********************************************************
   ...
   IMPLICIT NONE
       ...
       CALL iotk_write_end(unit,"RHAM")
            ...
            !added for recursion 
      CALL iotk_write_dat(unit,"WANCENTER", rave, ATTR=attr, COLUMNS=3, IERR=ierr) 
            IF (ierr/=0) CALL errore(subname,'writing rave',ABS(ierr))
            !
