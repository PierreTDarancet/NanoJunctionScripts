...
!
   ...
   PUBLIC                 :: lorentzian_transmittance_calculation 
   PUBLIC                 :: current_calculation 
			 
   CONTAINS 

!***********************************************
   SUBROUTINE  lorentzian_transmittance_calculation(conduct, hC, gamR, gamL, ene, dimx)
   !***********************************************
   ! 
  IMPLICIT NONE
      INTEGER, INTENT(in)   :: dimx

      REAL(dbl), INTENT(out) ::    conduct(dimx)
      COMPLEX(dbl), INTENT(in) ::  hC(dimx,dimx) 
   ...
   DO idiag=1,dimx
         !tmp(idiag) = gamL(idiag,idiag) * (ONE/(ene-hC(idiag,idiag) - CI * (EPS_m5 + gamR(idiag,idiag) + gamL(idiag,idiag) ) )  ) * gamR(idiag,idiag) * (ONE/(ene-hC(idiag,idiag) + CI*( gamR(idiag,idiag) + EPS_m5 + gamL(idiag,idiag) ) ) )
          tmp(idiag) =4.0* REAL(gamL(idiag,idiag)) * (ONE/(REAL(ene)-REAL( hC(idiag,idiag))  - CI * (EPS_m5 + REAL( gamR(idiag,idiag)) + REAL( gamL(idiag,idiag) ) ) )  ) * REAL( gamR(idiag,idiag) ) * (ONE/(REAL(ene)-REAL( hC(idiag,idiag)) + CI*( REAL( gamR(idiag,idiag) ) + EPS_m5 + REAL(gamL(idiag,idiag)) ) ) )
   ...
   DO idiag=1,dimx
      conduct(idiag) = REAL( tmp(idiag) )
   ENDDO
   !conductance = SUM(conduct(:))
   DO idiag=1,dimx
      IF (conduct(idiag) < ZERO) conduct(idiag)=ZERO
   ENDDO

       !
    END SUBROUTINE lorentzian_transmittance_calculation

!***********************************************
   SUBROUTINE  transmittance_calculation(conductance, conduct, gC, gamR, gamL, dimx)
   !***********************************************
   ! 
  IMPLICIT NONE
      INTEGER, INTENT(in)   :: dimx
