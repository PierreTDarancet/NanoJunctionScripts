...
!  datafile for hamiltonian right lead
   CHARACTER(nstrx) :: in_datafile_CC_1 = ' '
!  datafile for hamiltonian right lead
   CHARACTER(nstrx) :: in_datafile_CC_2 = ' '
!  datafile for hamiltonian right lead
!
...
! Mayou formula case
  ...
  INTEGER :: in_iter_CC_1 = 0
       !  recursion iter
  INTEGER :: in_iter_CC_2 = 0
       !  recursion iter
...
! Transmittance part
   ...
   NAMELIST / INPUT_CONDUCTOR /  ne, emin, emax, nprint, delta, delta_lead, &
                 ...
                 max_iter_CL, max_iter_CR, max_iter_CC, cut_chain, in_iter_CC_1, &
                 in_iter_CC_2, in_datafile_CC_1, in_datafile_CC_2
   !
...
! ... subroutines
!   PUBLIC :: dimR, dimL
   PUBLIC :: max_iter_CC, max_iter_CL, max_iter_CR, in_iter_CC_1, in_iter_CC_2
   PUBLIC :: cut_chain
...
!**********************************************************
   ...
   IMPLICIT NONE
     ...
     !      CALL errore(subname,'datafile_C unspecified',1)
      ...
      IF ( delta > EPS_m2 ) CALL errore(subname,'DELTA too large',4)
           ...
           !
      IF ( LEN_TRIM(in_datafile_CC_1) == 0 ) &
            CALL errore(subname,'datafile_CC_1 unspecified',5)
      IF ( LEN_TRIM(in_datafile_CC_2) == 0 ) &
            CALL errore(subname,'datafile_CC_2 unspecified',6)
 ...
 !     dimL = max_iter_L * dim_subspace
      itest = in_iter_CC_1 + in_iter_CC_2 + in_max_iter_L + in_max_iter_R - 1 - max_iter_R - max_iter_L - max_iter_CL - max_iter_CR
