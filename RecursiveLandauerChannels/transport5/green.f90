...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!DEBUG OPTION!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   COMPLEX(dbl)    :: rec_chain_debug(2, n_iter , dim_mat , dim_mat )
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!DEBUG OPTION!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!DEBUG OPTION!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  rec_chain_debug(2, : , : , : ) = 4.500 * CONE
  rec_chain_debug(1, : , : , : ) = CZERO
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!DEBUG OPTION!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Solve GN
...
!
    ...
    IF (cut_chain) THEN
         ...
         DO i_mat=1, dim_mat
            ...
            DO j_mat=1, dim_mat
               !
               spectra_min= rec_chain_debug(1, cut_iter(i_mat, j_mat), i_mat, j_mat) - 2 * ABS (rec_chain_debug(2,cut_iter(i_mat, j_mat), i_mat, j_mat))
               spectra_max= rec_chain_debug(1, cut_iter(i_mat, j_mat), i_mat, j_mat) + 2 * ABS (rec_chain_debug(2,cut_iter(i_mat, j_mat), i_mat, j_mat))
               !
               !
               IF ( ( REAL(ene) <= spectra_max ) .AND. ( REAL (ene) >=  spectra_min )) THEN
                     ...
                     ! b² - 4ac
                     !
                     delta = (rec_chain_debug(1, cut_iter(i_mat, j_mat), i_mat, j_mat) - REAL(ene) )**2 - 4 * (rec_chain_debug(2, cut_iter(i_mat, j_mat), i_mat, j_mat)**2)
                     IF (REAL(delta) > ZERO )  CALL errore(subname, 'delta > 0', INT(ABS(delta+1)) )
                     !
                     !
                     g_work(i_mat,j_mat)= (  ( ene - rec_chain_debug(1, cut_iter(i_mat, j_mat), i_mat, j_mat)  ) - CI* SQRT ( -delta ) ) / (2* (rec_chain_debug(2, cut_iter(i_mat, j_mat), i_mat, j_mat)**2))
                     !
                     !
                     ...
                     DO iter=1, (cut_iter(i_mat, j_mat)-1)
                        ...
                        !
                        work(i_mat,j_mat)= CONE / (ene - rec_chain_debug(1, shift_iter, i_mat, j_mat) - ((rec_chain_debug(2, shift_iter, i_mat, j_mat) **2) * work(i_mat,j_mat)))
                        !
                     ENDDO
                     !
                     !
    ...
    ELSE
         DO i_mat=1, dim_mat
            ...
            DO j_mat=1, dim_mat
               !
               spectra_min= rec_chain_debug(1, n_iter-1, i_mat, j_mat) - 2 * ABS (rec_chain_debug(2, n_iter-1, i_mat, j_mat))
               spectra_max= rec_chain_debug(1, n_iter-1, i_mat, j_mat) + 2 * ABS (rec_chain_debug(2, n_iter-1, i_mat, j_mat))
               !
               !
               IF ( ( REAL(ene) <= spectra_max ) .AND. ( REAL (ene) >=  spectra_min )) THEN
                     !
                     ! b² - 4ac
                     !
                     delta = (rec_chain_debug(1, n_iter-1, i_mat, j_mat) - ene)**2 - 4 * (rec_chain_debug(2, n_iter-1, i_mat, j_mat)**2)
                     IF (REAL(delta) > ZERO )  CALL errore(subname, 'delta > 0', INT(ABS(delta+1)) )
                     !
                     !
                     g_work(i_mat,j_mat)= (  -(rec_chain_debug(1, n_iter-1, i_mat, j_mat) - ene) - CI* SQRT ( -delta ) ) / (2* (rec_chain_debug(2, n_iter-1, i_mat, j_mat)**2))
                     !
                     !
                     work(i_mat,j_mat) = g_work(i_mat,j_mat)
                     !
