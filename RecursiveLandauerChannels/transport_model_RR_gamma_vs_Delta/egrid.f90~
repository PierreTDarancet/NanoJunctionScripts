...
!***********************************************
   SUBROUTINE egrid_init( biasmax,biasmin, bias )
   !***********************************************
  IMPLICIT NONE
   REAL(dbl),      INTENT(in)    :: biasmax
   REAL(dbl),      INTENT(in)    :: biasmin
   ...
   REAL(dbl)    :: biasref
       ...
       IF ( ABS(biasmax) - ABS(biasmin) >= ZERO  ) THEN
               biasref= biasmax
       ELSE 
               biasref= biasmin
       END IF 
       egrid(:) = ZERO
       emaxlocal = emax - ABS ( ( biasref )  / ( 2.0*ONE ) ) + ABS ( ( bias )  / ( 2.0*ONE ) ) ! This formulation should allow to deal with asymmetric bias cases: 
       eminlocal = emin + ABS ( ( biasref )  / ( 2.0*ONE ) ) - ABS ( ( bias )  / ( 2.0*ONE ) ) ! This formulation should allow to deal with asymmetric bias cases
       ...
       IF( emaxlocal <  eminlocal ) THEN 
            ...
            PRINT*, "=============STOPPING===================="
            PRINT*, "========================================="
            PRINT*, "Entered bias max     = ", biasmax, "  V"
            PRINT*, "Entered bias min     = ", biasmin, "  V"
            PRINT*, "Bias chosen as ref   = ", biasref, "  V"
            PRINT*, "Entered present bias = ", bias, "  V"
