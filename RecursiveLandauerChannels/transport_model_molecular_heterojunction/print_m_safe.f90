...
!***********************************************
   ...
   IMPLICIT NONE
       ...
       INTEGER, PARAMETER :: nspatialgrid=25
       REAL(dbl) :: Spatial_grid(nspatialgrid) 
       REAL(dbl) :: dspace, x_aux, x_aux2
       REAL(dbl)  :: PDOS_out(nspatialgrid,ne) 
       REAL(dbl)  :: eigenval(dimC)
       ! Local
       CHARACTER(10)      :: subname="print_PDOS"
       INTEGER  :: ierr, ie, i_dim, igrid
	   ...
	   DO ie = 1, ne
	       DO igrid = 1, nspatialgrid
		      IF (  (Spatial_grid(igrid) >= Gridmin ) .AND. (Spatial_grid(igrid) <= (Gridmin + Leadsize) )) THEN
		           PDOS_out(igrid,ie)= dosL(ie)  * f_L(ie)
		         ELSE IF  ((Spatial_grid(igrid) >= (Gridmax- Leadsize) ) .AND. (Spatial_grid(igrid) <= Gridmax ) ) THEN
		           PDOS_out(igrid,ie)= dosR(ie)* f_R(ie)
		         ELSE
		           PDOS_out(igrid,ie)= gammaL(ie,1,1)*gammaR(ie,1,1) *  ONE  /( (egrid(ie) - eigenval(1) )**(2) + ( ( gammaL(ie,1,1)+gammaR(ie,1,1) )**2)/2.0 )
		           !PDOS_out(igrid,ie)=  ONE  /( (egrid(ie) - eigenval(1) )**(2) + ( ( gammaL(ie,1,1)+gammaR(ie,1,1) )**2)/2.0 )
		      ENDIF 
	       ENDDO
	   ENDDO
      ELSE IF  ( dimC == 2 ) THEN 
	       ...
	       DO igrid = 1, nspatialgrid
		      IF (  (Spatial_grid(igrid) >= Gridmin ) .AND. (Spatial_grid(igrid) <= (Gridmin + Leadsize - deltagraph) )) THEN
		         ...
		         ELSE IF  ( ( Spatial_grid(igrid) > x_aux+ deltagraph ) .AND. ( Spatial_grid(igrid) < (Gridmax- Leadsize - deltagraph) )) THEN
		           PDOS_out(igrid,ie)= gammaL(ie,2,2)*gammaR(ie,2,2) * ONE /( (egrid(ie) - eigenval(2) )**(2) + ( ( gammaL(ie,2,2)+gammaR(ie,2,2) )**2)/2.0 )
		           !PDOS_out(igrid,ie)= ONE /( (egrid(ie) - eigenval(2) )**(2) + ( ( gammaL(ie,2,2)+gammaR(ie,2,2) )**2)/2.0 )
		      ENDIF 
	       ENDDO
	   ENDDO
	       ...
	       DO igrid = 1, nspatialgrid
		      IF (  (Spatial_grid(igrid) >= Gridmin ) .AND. (Spatial_grid(igrid) <= (Gridmin + Leadsize - deltagraph) )) THEN
		         ...
		         ELSE IF  ( ( Spatial_grid(igrid) > x_aux2 + deltagraph) .AND. ( Spatial_grid(igrid) < (Gridmax- Leadsize - deltagraph) ) ) THEN
		           PDOS_out(igrid,ie)= gammaL(ie,3,3) * gammaR(ie,3,3) * ONE /( (egrid(ie) - eigenval(3) )**(3) + ( ( gammaL(ie,3,3)+gammaR(ie,3,3) )**2)/2.0 )
		           !PDOS_out(igrid,ie)= ONE /( (egrid(ie) - eigenval(3) )**(3) + ( ( gammaL(ie,3,3)+gammaR(ie,3,3) )**2)/2.0 )
		      ENDIF 
	       ENDDO
	   ENDDO
	       ...
	       DO igrid = 1, nspatialgrid
		      IF (  (Spatial_grid(igrid) >= Gridmin ) .AND. (Spatial_grid(igrid) <= (Gridmin + Leadsize - deltagraph) )) THEN
		         ...
		         ELSE IF  ((Spatial_grid(igrid) >= (Gridmax- Leadsize+ deltagraph) ) .AND. (Spatial_grid(igrid) <= Gridmax ) ) THEN
		           PDOS_out(igrid,ie)= dosR(ie)* f_R(ie)
		         ELSE IF  ( ( Spatial_grid(igrid) > (Gridmin + Leadsize+ deltagraph) ) .AND. ( Spatial_grid(igrid) <= x_aux - deltagraph )) THEN
		           PDOS_out(igrid,ie)=  gammaL(ie,1,1) * gammaR(ie,1,1) * ONE /( (egrid(ie) - eigenval(1) )**(2) + ( ( gammaL(ie,1,1)+gammaR(ie,1,1) )**2)/2.0 ) + gammaL(ie,2,2) * gammaR(ie,2,2) * ONE /( (egrid(ie) - eigenval(2) )**(2) + ( ( gammaL(ie,2,2)+gammaR(ie,2,2) )**2)/2.0 )
		           !PDOS_out(igrid,ie)= ONE /( (egrid(ie) - eigenval(1) )**(2) + ( ( gammaL(ie,1,1)+gammaR(ie,1,1) )**2)/2.0 ) + ONE /( (egrid(ie) - eigenval(2) )**(2) + ( ( gammaL(ie,2,2)+gammaR(ie,2,2) )**2)/2.0 )
		         ELSE IF  ( ( Spatial_grid(igrid) > x_aux + deltagraph) .AND. ( Spatial_grid(igrid) < (Gridmax- Leadsize - deltagraph) )) THEN
		           PDOS_out(igrid,ie)=  gammaL(ie,3,3)*gammaR(ie,3,3)* ONE /( (egrid(ie) - eigenval(3) )**(2) + ( ( gammaL(ie,3,3)+gammaR(ie,3,3) )**2)/2.0 ) + gammaL(ie,4,4)*gammaR(ie,4,4)*ONE /( (egrid(ie) - eigenval(4) )**(2) + ( ( gammaL(ie,4,4)+gammaR(ie,4,4) )**2)/2.0 )
		      ENDIF 
	       ENDDO
	   ENDDO
	   ...
	   DO ie = 1, ne
	       DO igrid = 1, nspatialgrid
		      IF (  (Spatial_grid(igrid) >= Gridmin ) .AND. (Spatial_grid(igrid) <= (Gridmin + Leadsize - deltagraph) )) THEN
		         ...
		         ELSE IF  ( ( Spatial_grid(igrid) > x_aux + deltagraph) .AND. ( Spatial_grid(igrid) < x_aux2 - deltagraph ) )THEN
                           DO i_dim=1, dimC-2
   			           PDOS_out(igrid,ie)= PDOS_out(igrid,ie) + ONE /( (egrid(ie) - eigenval(i_dim) )**(2) + ( ( gammaL(ie,i_dim,i_dim)+gammaR(ie,i_dim,i_dim) )**2)/2.0 )
                           ENDDO
		         ELSE IF  ( ( Spatial_grid(igrid) > x_aux2 + deltagraph ) .AND. ( Spatial_grid(igrid) < (Gridmax- Leadsize - deltagraph) ) ) THEN
		           PDOS_out(igrid,ie)= gammaL(ie,dimC,dimC) * gammaR(ie,dimC,dimC)  * ONE /( (egrid(ie) - eigenval(dimC) )**(2) + ( ( gammaL(ie,dimC,dimC)+gammaR(ie,dimC,dimC) )**2)/2.0 ) +  gammaL(ie,(dimC-1),(dimC-1)) * gammaR(ie,(dimC-1),(dimC-1)) * ONE /( (egrid(ie) - eigenval((dimC-1)) )**(2) + ( ( gammaL(ie,(dimC-1),(dimC-1))+gammaR(ie,(dimC-1),(dimC-1)) )**2)/2.0 )

		           !PDOS_out(igrid,ie)= ONE /( (egrid(ie) - eigenval(dimC) )**(2) + ( ( gammaL(ie,dimC,dimC)+gammaR(ie,dimC,dimC) )**2)/2.0 ) + ONE /( (egrid(ie) - eigenval((dimC-1)) )**(2) + ( ( gammaL(ie,(dimC-1),(dimC-1))+gammaR(ie,(dimC-1),(dimC-1)) )**2)/2.0 )
		      ENDIF 
	       ENDDO
	   ENDDO
      ENDIF
 ...
 !!!!!!OUt
   ...
   DO ie = 1, ne
       DO igrid = 1, nspatialgrid
                 WRITE (Fil, '(3(f15.9))' ) egrid(ie) , Spatial_grid(igrid),   (PDOS_out(igrid,ie) )
       ENDDO
       WRITE (Fil, '(2(e15.9))' )
   ENDDO
   CLOSE( Fil )
