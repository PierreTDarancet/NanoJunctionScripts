...
! max number of iteration
   ...
   CHARACTER(nstrx) :: form_datafile_C=''
!
   CHARACTER(nstrx) :: in_datafile_C = ' '
!  datafile for hamiltonian conductor



   LOGICAL ::  import_gamma = .FALSE.
      !!!!!!!!!IF import gamma =.TRUE.
   CHARACTER(nstrx) :: import_gamma_datafile_L = ' '
!  datafile for gamma
   CHARACTER(nstrx) :: import_sigma_datafile_L = ' '
!  datafile for gamma
   CHARACTER(nstrx) :: import_gamma_datafile_R = ' '
!  datafile for gamma
   CHARACTER(nstrx) :: import_sigma_datafile_R = ' '
!  datafile for gamma


   LOGICAL ::  import_gamma_tilde = .FALSE.
      !!!!!!!!!IF import gamma_tilde =.TRUE.
   CHARACTER(nstrx) :: import_gamma_tilde_datafile_L = ' '
!  datafile for gamma
   CHARACTER(nstrx) :: import_sigma_tilde_datafile_L = ' '
!  datafile for gamma
   CHARACTER(nstrx) :: import_gamma_tilde_datafile_R = ' '
!  datafile for gamma
   CHARACTER(nstrx) :: import_sigma_tilde_datafile_R = ' '
!  datafile for gamma



   CHARACTER(nstrx) :: method_for_sigma_L = ' '
!  calcul des terminaisons de chaines
!  analytique
   COMPLEX(dbl) :: diag_GL = CZERO
!  Recursion & WanT
   ...
   CHARACTER(nstrx) :: method_for_sigma_R = ' '
!  calcul des terminaisons de chaines
!  analytique
   COMPLEX(dbl) :: diag_GR = CZERO
!  Recursion & WanT
...
! Transmittance part
   ...
   NAMELIST / INPUT_CONDUCTOR /  ne, emin, emax, nprint, delta, delta_lead, &
                 ...
                 max_iter_CL, max_iter_CR, max_iter_CC, cut_chain
