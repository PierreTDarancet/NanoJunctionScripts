...
!*********************************************
   ...
   USE dim_variable_module,        ONLY : n_orb, dim_subspace, dim_recursion
   ...
   USE distance_module,           ONLY : ene_onsite, nb_first, id_first, ene_first, nb_second, id_second, ene_second, &
                                         nb_third, id_third, ene_third
   ...
   COMPLEX(dbl), ALLOCATABLE ::  recursion_state_1(:,:), recursion_state_2(:,:), recursion_state_3(:,:)

!    COMPLEX(dbl), ALLOCATABLE ::   recursion_state_0(:,:), recursion_state(:,:,:)
   !
   ...
   PUBLIC :: A_matrix_0, B_matrix_0
!    PUBLIC :: recursion_state_0, recursion_state
   PUBLIC :: A_matrix, B_matrix
   PUBLIC :: recursion_state_1, recursion_state_2, recursion_state_3
...
!         IF( ierr /=0 ) CALL errore(subname, 'allocating overlap_matrix', ABS(ierr) )
   ALLOCATE ( recursion_state_1(dim_subspace, dim_recursion), STAT=ierr )
        IF( ierr /=0 ) CALL errore(subname, 'allocating recursion_state_1', ABS(ierr) )
   ALLOCATE ( recursion_state_2(dim_subspace, dim_recursion), STAT=ierr )
        IF( ierr /=0 ) CALL errore(subname, 'allocating recursion_state_2', ABS(ierr) )
   ALLOCATE ( recursion_state_3(dim_subspace, dim_recursion), STAT=ierr )
        IF( ierr /=0 ) CALL errore(subname, 'allocating recursion_state_3', ABS(ierr) )
...
!       ENDIF
     IF ( ALLOCATED( recursion_state_1  ) ) THEN
           DEALLOCATE ( recursion_state_1 , STAT=ierr )
           IF( ierr /=0 ) CALL errore(subname, 'deallocating recursion_state_1', ABS(ierr) )
      ENDIF
     IF ( ALLOCATED( recursion_state_2  ) ) THEN
           DEALLOCATE ( recursion_state_2 , STAT=ierr )
           IF( ierr /=0 ) CALL errore(subname, 'deallocating recursion_state_2', ABS(ierr) )
      ENDIF
     IF ( ALLOCATED( recursion_state_3  ) ) THEN
           DEALLOCATE ( recursion_state_3 , STAT=ierr )
           IF( ierr /=0 ) CALL errore(subname, 'deallocating recursion_state_3', ABS(ierr) )
      ENDIF
...
!     recursion_state(:,:,:)=CZERO
    recursion_state_1(:,:)=CZERO
    recursion_state_2(:,:)=CZERO
    recursion_state_3(:,:)=CZERO
...
!*******************************************************************
   ...
   IF (lesser) THEN
      ...
      icount=0
      DO  i_orb=1,n_orb
         IF ( orb(i_orb)%coord(transport_dir) <= limit ) THEN
            subrecur2tot(icount+1)=i_orb
            icount=icount+1
         ENDIF
      ENDDO
      !
   ...
   DO  icount=1,dim_recursion
      !
      orb_recursion(icount)%nature = orb(subrecur2tot(icount))%nature
      orb_recursion(icount)%coord(:) = orb(subrecur2tot(icount))%coord(:)
      !wannier_coordinates_sub_recursion(:,icount) =orb(subrecur2tot(icount))%coord(:)
      orb_recursion(icount)%init = .TRUE.
      !
   ENDDO
...
!     ENDDO
    ...
    DO isub = 1, dim_subspace
        DO irecur =1, dim_recursion
           IF  ( wan_num(isub) == subrecur2tot(irecur) ) THEN
               recursion_state_1(isub,irecur) = CONE
           ELSE 
               recursion_state_1(isub,irecur) = CZERO
           ENDIF
        ENDDO
    ENDDO
...
!       PRINT*, '-----------------------------------------------------------------'
!       PRINT*, '                        nb_first'
!       PRINT*, nb_first(i_recur)
!       PRINT*, '-----------------------------------------------------------------'
!       PRINT*, '                        ID_first'
!       PRINT*, id_first(:,i_recur)
!       PRINT*, '-----------------------------------------------------------------'
!       PRINT*, '                        ENE_first'
!       PRINT*, ene_first(:,i_recur)
!       PRINT*, '-----------------------------------------------------------------'
!       PRINT*, '-----------------------------------------------------------------'
!     ENDDO


    !PRINT*, 'Debut, première iteration'

     !
!      CALL calculate_A_matrix(nb_first, id_first, ene_onsite, ene_first, recursion_state_0(:,:), A_matrix_0(:,:) , debug_mode) 
     !
     !
     CALL calculate_A_matrix(nb_first, id_first, ene_onsite, ene_first, recursion_state_1(:,:), A_matrix_0(:,:) , debug_mode) 
     !
    ...
    SELECT CASE( TRIM(calculation_mode) )
       CASE ("no_condition")
     !
!            CALL calculate_recursion_state(nb_first, id_first, ene_onsite, ene_first, recursion_state_0(:,:), A_matrix_0(:,:), aux1(:,:), &
!                                             aux2(:,:), recursion_state(1,:,:) , debug_mode)

           CALL calculate_recursion_state(nb_first, id_first, ene_onsite, ene_first, recursion_state_1(:,:), A_matrix_0(:,:), aux1(:,:), &
                                            aux2(:,:), recursion_state_2(:,:) , debug_mode)
...
!           CALL normalize_recursion_state( recursion_state(1,:,:), B_matrix_0(:,:), norm(1), numerical_cut_off, debug_mode )
      !print*, recursion_state(1,:,:)
          ...
          CALL normalize_recursion_state( recursion_state_2(:,:), B_matrix_0(:,:), norm(1), numerical_cut_off, debug_mode )
      !print*, recursion_state(1,:,:)

       CASE ("change_V")
            CALL calculate_recursion_state(nb_first, id_first, ene_onsite, ene_first, recursion_state_1(:,:), A_matrix_0(:,:), &
                                aux1(:,:), aux2(:,:), recursion_state_2(:,:), debug_mode )
     !
            CALL normalize_recursion_state_rpp( recursion_state_2(:,:), B_matrix_0(:,:), norm(1), numerical_cut_off, debug_mode )
       CASE ("diagonal_A")
            CALL calculate_recursion_state_diag_A(nb_first, id_first, ene_onsite, ene_first, recursion_state_1(:,:), A_matrix_0(:,:), &
                                aux1(:,:), aux2(:,:), recursion_state_2(:,:), B_matrix_0(:,:), norm(1), numerical_cut_off, debug_mode )

      !
       CASE ("diag_A_cond_V")
            CALL calculate_recursion_state_diag_A_rpp(nb_first, id_first, ene_onsite, ene_first, recursion_state_1(:,:), A_matrix_0(:,:), &
                                aux1(:,:), aux2(:,:), recursion_state_2(:,:), B_matrix_0(:,:), norm(1), numerical_cut_off, debug_mode )

      !
       CASE ("diag_A_sum_B")
            CALL calculate_recursion_state_diag_A_sum_B(nb_first, id_first, ene_onsite, ene_first, recursion_state_1(:,:), A_matrix_0(:,:), &
                                aux1(:,:), aux2(:,:), recursion_state_2(:,:), B_matrix_0(:,:), norm(1), numerical_cut_off, debug_mode )
      !
       CASE ("diag_A_sgn_B")
            CALL calculate_recursion_state_diag_A_sgn_B(nb_first, id_first, ene_onsite, ene_first, recursion_state_1(:,:), A_matrix_0(:,:), &
                                aux1(:,:), aux2(:,:), recursion_state_2(:,:), B_matrix_0(:,:), norm(1), numerical_cut_off, debug_mode )
      !
       CASE ("diag_B_B_dagger")
            CALL calculate_recursion_state_diag_B_B_dagger(nb_first, id_first, ene_onsite, ene_first, recursion_state_1(:,:), A_matrix_0(:,:), &
                                aux1(:,:), aux2(:,:), recursion_state_2(:,:), B_matrix_0(:,:), norm(1), numerical_cut_off, debug_mode )
      !
...
!      ENDDO
     !
     CALL calculate_A_matrix(nb_first, id_first, ene_onsite, ene_first, recursion_state_2(:,:) ,  A_matrix(1,:,:), debug_mode ) 
     !

    SELECT CASE( TRIM(calculation_mode) )
       CASE ("no_condition")
     !
           CALL calculate_recursion_state(nb_first, id_first, ene_onsite, ene_first,  recursion_state_2(:,:), A_matrix(1,:,:), B_matrix_0(:,:), &
                                            recursion_state_1(:,:), recursion_state_3(:,:), debug_mode )
     !
          CALL normalize_recursion_state( recursion_state_3(:,:),  B_matrix(1,:,:), norm(2), numerical_cut_off, debug_mode  )
     !
       CASE ("change_V")
            CALL calculate_recursion_state(nb_first, id_first, ene_onsite, ene_first, recursion_state_2(:,:), A_matrix(1,:,:), &
                         B_matrix_0(:,:), recursion_state_1(:,:), recursion_state_3(:,:), debug_mode )
     !
            CALL normalize_recursion_state_rpp(recursion_state_3(:,:),  B_matrix(1,:,:), norm(2), numerical_cut_off, debug_mode )
     !
       CASE ("diagonal_A")
            CALL calculate_recursion_state_diag_A(nb_first, id_first, ene_onsite, ene_first, recursion_state_2(:,:), A_matrix(1,:,:), &
                         B_matrix_0(:,:), recursion_state_1(:,:), recursion_state_3(:,:),  B_matrix(1,:,:), norm(2), numerical_cut_off, debug_mode  )
     !
       CASE ("diag_A_cond_V")
            CALL calculate_recursion_state_diag_A_rpp(nb_first, id_first, ene_onsite, ene_first, recursion_state_2(:,:), A_matrix(1,:,:), &
                         B_matrix_0(:,:), recursion_state_1(:,:), recursion_state_3(:,:),  B_matrix(1,:,:), norm(2), numerical_cut_off, debug_mode  )
      !
       CASE ("diag_A_sum_B")
            CALL calculate_recursion_state_diag_A_sum_B(nb_first, id_first, ene_onsite, ene_first, recursion_state_2(:,:), A_matrix(1,:,:), &
                         B_matrix_0(:,:), recursion_state_1(:,:), recursion_state_3(:,:),  B_matrix(1,:,:), norm(2), numerical_cut_off, debug_mode  )
      !
       CASE ("diag_A_sgn_B")
            CALL calculate_recursion_state_diag_A_sgn_B(nb_first, id_first, ene_onsite, ene_first, recursion_state_2(:,:), A_matrix(1,:,:), &
                         B_matrix_0(:,:), recursion_state_1(:,:), recursion_state_3(:,:),  B_matrix(1,:,:), norm(2), numerical_cut_off, debug_mode  )
      !
       CASE ("diag_B_B_dagger")
            CALL calculate_recursion_state_diag_B_B_dagger(nb_first, id_first, ene_onsite, ene_first, recursion_state_2(:,:), A_matrix(1,:,:), &
                         B_matrix_0(:,:), recursion_state_1(:,:), recursion_state_3(:,:),  B_matrix(1,:,:), norm(2), numerical_cut_off, debug_mode  )
      !
     ...
     CALL summary_recursion_input(stdout, A_matrix_0, B_matrix_0, recursion_state_1, &
                                  wan_num,  subrecur2tot, norm(1),  alloc)
      !
...
!
  ...
  recursion_state_1(:,:) = recursion_state_2(:,:)
  recursion_state_2(:,:) = recursion_state_3(:,:)
  recursion_state_3(:,:) = CZERO
  iteration_loop : &
  DO iter = 1, max_iter-1
     ...
     iteration = iter+1
     !
     CALL calculate_A_matrix(nb_first, id_first, ene_onsite, ene_first, recursion_state_2(:,:) , A_matrix(iteration,:,:), debug_mode ) 
     !
     !
     SELECT CASE( TRIM(calculation_mode) )
       CASE ("no_condition")
            CALL calculate_recursion_state(nb_first, id_first, ene_onsite, ene_first, recursion_state_2(:,:), A_matrix(iteration,:,:), &
                         B_matrix(iteration-1,:,:), recursion_state_1(:,:), recursion_state_3(:,:), debug_mode )
     !

     !
     !
     ! Ici 3 possibilites : laisser tel quel et toucher à la convergence
              CALL normalize_recursion_state(  recursion_state_3(:,:), B_matrix(iteration,:,:), norm(iteration+1), numerical_cut_off, debug_mode )
       CASE ("change_V")
            CALL calculate_recursion_state(nb_first, id_first, ene_onsite, ene_first, recursion_state_2(:,:), A_matrix(iteration,:,:), &
                         B_matrix(iteration-1,:,:), recursion_state_1(:,:), recursion_state_3(:,:), debug_mode )
     !
            CALL normalize_recursion_state_rpp(  recursion_state_3(:,:), B_matrix(iteration,:,:), norm(iteration+1), numerical_cut_off, debug_mode )
       CASE ("diagonal_A")
            CALL calculate_recursion_state_diag_A(nb_first, id_first, ene_onsite, ene_first, recursion_state_2(:,:), A_matrix(iteration,:,:), &
                         B_matrix(iteration-1,:,:), recursion_state_1(:,:), recursion_state_3(:,:), &
                         B_matrix(iteration,:,:), norm(iteration+1), numerical_cut_off, debug_mode )
     !
       CASE ("diag_A_cond_V")
            CALL calculate_recursion_state_diag_A_rpp(nb_first, id_first, ene_onsite, ene_first, recursion_state_2(:,:), A_matrix(iteration,:,:), &
                         B_matrix(iteration-1,:,:), recursion_state_1(:,:), recursion_state_3(:,:), &
                         B_matrix(iteration,:,:), norm(iteration+1), numerical_cut_off, debug_mode )
     !
       CASE ("diag_A_sum_B")
            CALL calculate_recursion_state_diag_A_sum_B(nb_first, id_first, ene_onsite, ene_first, recursion_state_2(:,:), A_matrix(iteration,:,:), &
                         B_matrix(iteration-1,:,:), recursion_state_1(:,:), recursion_state_3(:,:), &
                         B_matrix(iteration,:,:), norm(iteration+1), numerical_cut_off, debug_mode )
     !
       CASE ("diag_A_sgn_B")
            CALL calculate_recursion_state_diag_A_sgn_B(nb_first, id_first, ene_onsite, ene_first, recursion_state_2(:,:), A_matrix(iteration,:,:), &
                         B_matrix(iteration-1,:,:), recursion_state_1(:,:), recursion_state_3(:,:), &
                         B_matrix(iteration,:,:), norm(iteration+1), numerical_cut_off, debug_mode )
     !
       CASE ("diag_B_B_dagger")
            CALL calculate_recursion_state_diag_B_B_dagger(nb_first, id_first, ene_onsite, ene_first, recursion_state_2(:,:), A_matrix(iteration,:,:), &
                         B_matrix(iteration-1,:,:), recursion_state_1(:,:), recursion_state_3(:,:), &
                         B_matrix(iteration,:,:), norm(iteration+1), numerical_cut_off, debug_mode )
     !
...
!*******************************************************************
   ...
   !*******************************************************************
      ...
      CHARACTER( LEN=nstrx )  :: filename
       ...
       CALL iotk_write_attr(attr,"n_orb_total",n_orb,FIRST=.TRUE.) 
...
!*******************************************************************
   ...
   !*******************************************************************
       ...
       CHARACTER(nstrx)   :: attr
       INTEGER            :: iter, i_sub, i_orb, i_rec
       INTEGER            :: ierr
       CHARACTER( LEN=nstrx )  :: filename
       REAL(dbl), ALLOCATABLE :: wannier_coord_aux(:,:)
       REAL(dbl), ALLOCATABLE :: wannier_coord_rec_aux(:,:)
       ...
       DO i_sub=1, dim_subspace
         ...
         CALL iotk_write_attr(attr,"COOR",orb(wan_num(i_sub))%coord(:), IERR=ierr) 
       ...
       CALL iotk_write_dat(cen_unit,"REC2TOT", subrecur2tot(:))
        ...
        ALLOCATE ( wannier_coord_aux(3,n_orb), STAT=ierr )
            IF( ierr /=0 ) CALL errore(subname, 'allocating wannier_coord_aux', ABS(ierr) )
        ALLOCATE ( wannier_coord_rec_aux(3,dim_recursion), STAT=ierr )
            IF( ierr /=0 ) CALL errore(subname, 'allocating wannier_coord_rec_aux', ABS(ierr) )


       DO i_orb=1, n_orb
            !
            wannier_coord_aux(:,i_orb) = orb(i_orb)%coord(:)
            !
       ENDDO
      ...
      !
       ...
       DO i_rec=1, dim_recursion
            !
            wannier_coord_rec_aux(:,i_rec) = orb_recursion(i_rec)%coord(:)
            !
       ENDDO


      CALL iotk_write_dat(cen_unit,"INITCENTER", wannier_coord_aux, COLUMNS=3, IERR=ierr) 
            IF (ierr/=0) CALL errore(subname,'writing wannier_coordinates',ABS(ierr))


       CALL iotk_write_dat(cen_unit,"RECCENTER", wannier_coord_rec_aux,  COLUMNS=3, IERR=ierr) 
     ...
     IF ( ALLOCATED( wannier_coord_aux ) ) THEN
           DEALLOCATE ( wannier_coord_aux, STAT=ierr )
           IF( ierr /=0 ) CALL errore(subname, 'deallocating wannier_coord_aux', ABS(ierr) )
      ENDIF
     IF ( ALLOCATED( wannier_coord_rec_aux ) ) THEN
           DEALLOCATE ( wannier_coord_aux, STAT=ierr )
           IF( ierr /=0 ) CALL errore(subname, 'deallocating wannier_coord_rec_aux', ABS(ierr) )
      ENDIF
